...
{
    public partial class KimlikNumarasi : Form
    {
        public KimlikNumarasi()
        {
            InitializeComponent();
        }
        ...
        private void btnKontrol_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                if (tcNo.Length == 11)
                {
                    ...
                    {
                        lblKontrol.Text = "Tc Kimlik Numaranızın ilk hanesi 0 olamaz.";
                    }
                }
                else
                {
                    lblKontrol.Text = "Tc Kimlik Numarınız 11 haneli olmak zorunda.Eksik ya da fazla değer girdiniz.";
                }
                if (((toplam2 * 7) - toplam3) % 10 == Convert.ToInt32(tcNo[9].ToString()) && toplam % 10 == Convert.ToInt32(tcNo[10].ToString()))
                {
                    lblKontrol.Text = ("Tc Kimlik Numarası Doğru.");
                }
                else
                {
                    lblKontrol.Text = ("Tc Kimlik Numarası Yanlış!");
                }
        ...
        private void btnÜret_Click(object sender, EventArgs e)
        {
           ...
           // txxKimlikGiris.Text=a.ToString();
            string deneme="";
            foreach (var item in a)
            {
                deneme += item;
            }
            txxKimlikGiris.Text = deneme;
           
           
        }

        private void KimlikNumarasi_Load(object sender, EventArgs e)
        {
